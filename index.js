...
app.get("/", async (req, res) => {
  ...
  const botTag = client.user?.tag || "Desconectado";
...
<head>
  ...
  <title>Bot Hierarquia HP - Status</title>
  <style>
    :root {
      --bg: #0f172a;
      --card: #1e293b;
...
<body>
  <div class="container">
    <div class="header">
      <div>
        <h1 style="margin:0; font-size: 1.5rem;">\u{1F916} Bot Hierarquia HP</h1>
        <p style="margin: 0.25rem 0 0 0; color: var(--text-muted); font-size: 0.875rem;">Servidor Standalone Discord API & Sync</p>
      </div>
      <div class="status-badge ${isReady ? "" : "offline"}">
        <span class="dot"></span>
        ${isReady ? `Online (${botTag})` : "Aguardando Token / Offline"}
      </div>
    </div>
    ...
    <div class="grid" style="margin-bottom: 1.5rem;">
      <div class="stat-box">
        ...
        <div class="stat-label">Membros Mapeados</div>
      </div>
      <div class="stat-box">
        <div class="stat-value">${database.estatisticas.sincronizacoes}</div>
        <div class="stat-label">Sincroniza\xE7\xF5es</div>
      </div>
      <div class="stat-box">
        <div class="stat-value">${database.advertencias.length}</div>
        <div class="stat-label">Advert\xEAncias</div>
      </div>
    </div>

    <div class="card">
      <h2>\u{1F4CA} Quadro de Hierarquia Atual</h2>
      <pre>${textoHierarquia}</pre>
      <div class="actions">
        <button onclick="fetch('/api/sincronizar', {method:'POST'}).then(r=>r.json()).then(d=>alert(d.message))">\u26A1 Sincronizar Agora</button>
        <button onclick="fetch('/api/atualizar-quadro', {method:'POST'}).then(r=>r.json()).then(d=>alert(d.message))" style="background:#3b82f6; color:#fff;">\u{1F4CC} Enviar / Atualizar Quadro</button>
      </div>
    </div>
  </div>
</body>
...
app.listen(Number(PORT), "0.0.0.0", () => {
  console.log(`\u{1F680} Bot Hierarquia rodando na porta ${PORT}`);
});
export {
  extrairIdFiveM,
  formatarLinhaMembro,
